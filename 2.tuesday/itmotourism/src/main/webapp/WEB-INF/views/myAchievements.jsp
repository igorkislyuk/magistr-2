        ...
        <div class="row justify-content-md-center">
            <div class="mbr-white col-md-10">
                ...
                <%CompetitionsRepository competitionsRepository = (CompetitionsRepository)session.getAttribute("events");
                    ...
                    int k = 0;
                    List<Places> placesList = placesRepository.findByPeopleId(peopleRepository.findByUserName((String)session.getAttribute("userName")).getId());
                    for(Places places: placesList){
                        if(places.getPlace()!=0)k++;
                    }
                    if(k==0){
                %>
                <%="К сожалению, вы не добились ничего"%>
                <%}else{%>
                <%="                        <table>\n" +
                        "                            <tr>\n" +
                        "                                <th>Событие</th>\n" +
                        ...
                        "                            </tr>\n"%>
                <%for(Places places: placesList){
                    if(places.getPlace()!=0){
                        Competitions competitions = competitionsRepository.findById(places.getCompetitionId());%>
                <%="<th>"+competitions.getName()+"("+competitions.getDate()+")</th>" +
                        "<th>"+places.getPlace()+"</th>"%>
                <%}}%>
                <%="                        </table>"%>
                <%}%>
                </p>
